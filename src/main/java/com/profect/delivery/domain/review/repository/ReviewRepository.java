...
public interface ReviewRepository extends JpaRepository<Review, UUID> { //<이 repo가 다룰 엔티티 클래스, pk의 타입>
    ...
    // 리뷰 조회 시, username 포함
    @Query("""
    SELECT new com.profect.delivery.domain.review.dto.ReviewResponseDto(
        r.reviewId,
        r.userId,
        u.username,
        r.comment,
    ...
    WHERE r.storeId = :storeId AND r.reviewId = :reviewId AND r.isPublic = true
""")
    Optional<ReviewResponseDto> findReviewWithUsernameByStoreIdAndReviewId(UUID storeId, UUID reviewId);
